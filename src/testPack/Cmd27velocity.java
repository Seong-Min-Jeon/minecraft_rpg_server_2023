...
public class Cmd27velocity implements CommandExecutor {
	...
	public Entity[] getTargets(CommandSender sender, String name) {
		Entity[] ents;
		Location loc = null;
		if (sender instanceof Player) {
		...
		if (name.startsWith("@s")) {
			...
			ArrayList<Entity> listOfValidEntities = new ArrayList<>();
			int C = getLimit(name);
			...
			if (!usePlayers) {
				ea.clear();
				for (World w : getAcceptedWorldsFullString(loc, name)) {
...
//					    }
			...
			for (World w : getAcceptedWorldsFullString(loc, name)) {
				if (hasTag(SelectorType.TYPE, name)) {
					for (Entity e : w.getEntities()) {
						boolean good = true;
						for (String tag : tags) {
							if (!canBeAccepted(tag, e, loc)) {
			...
			ents = new Entity[]{Bukkit.getPlayer(name)};
		}
		return ents;
	}
	...
	private static boolean isType(String arg, Entity e) {
		...
		if (invert != e.getType().name().equalsIgnoreCase(type.split(":")[1]))
			return true;
		return false;
